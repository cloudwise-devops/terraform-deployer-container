...
ENV KUBEDOG_VERSION="v0.6.1"
COPY google-sdk.repo /etc/yum/repos.d/google-sdk.repo
RUN yum update -y && yum install epel-release wget git sudo -y && \
wget https://storage.yandexcloud.net/kubedog/targets/releases/${KUBEDOG_VERSION}/kubedog-linux-amd64-${KUBEDOG_VERSION} -O /usr/bin/kubedog && \
